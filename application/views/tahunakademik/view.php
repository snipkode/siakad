<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
  <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-5">
    <div class="flex items-center gap-3">
      <span class="hidden h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500 to-green-600 text-white shadow-md sm:inline-flex">
        <i class="fa fa-calendar"></i>
      </span>
      <div>
        <h3 class="text-sm font-bold text-slate-800">Data <?php echo meta_mode_label('label_tahun'); ?></h3>
        <p class="text-xs text-slate-500">
          Total
          <span id="total-tahunakademik" class="inline-flex items-center justify-center rounded-full bg-emerald-100 text-emerald-700 px-2 py-0.5 text-[11px] font-bold">0</span>
          terdaftar
        </p>
      </div>
    </div>
    <div class="flex flex-wrap gap-2 dt-actions">
      <?php
    echo anchor('tahunakademik/add', '<i class="fa fa-plus"></i> Tambah Data', array('class'=>'inline-flex items-center gap-1.5 rounded-xl bg-sky-600 px-3 py-2 text-xs font-semibold text-white shadow-sm hover:bg-sky-700'));
  ?>
    </div>
  </div>
  ...
  $(document).ready(function () {
    var t = $('#mytable').DataTable({
      ...
      "initComplete": function (settings, json) {
        $('#total-tahunakademik').text(json.recordsTotal);
        var $sf = $("#mytable_wrapper").parents(".rounded-2xl").first().find(".dataTables_filter input");
        $sf.attr("placeholder", "Cari <?php echo mb_strtolower(meta_mode_label('label_tahun')); ?>...");
        $sf.css("min-width", "200px");
      },
      "columns": [
{ "data": null, "width": "45px", "class": "text-center", "orderable": false },
        { "data": "tahun_akademik", "class": "text-center font-mono" },
    ...
    t.on('draw.dt', function () {
      $('#total-tahunakademik').text(t.page.info().recordsTotal);
