...
<div class="mx-auto max-w-3xl space-y-4 sm:space-y-5">
  ...
  <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
    <div class="mb-3 sm:mb-4">
      <h2 class="text-sm font-bold text-slate-800 sm:text-base">Mode Instansi</h2>
      <p class="mt-0.5 text-[11px] text-slate-500 sm:text-xs">Pilih KAMPUS, SMA, SMP, SD, atau TK. Label, menu, struktur & laporan menyesuaikan otomatis.</p>
    </div>
    ...
    <div class="grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-3 lg:grid-cols-5">
      <?php foreach ($modes as $m):
        ...
        <button type="button" onclick="pilihMode('<?php echo $m['kd_mode']; ?>')"
          class="rounded-xl border px-3 py-2.5 text-left transition sm:rounded-2xl sm:p-4 <?php echo $aktif
            ? 'border-sky-500 bg-sky-50 shadow-sm ring-2 ring-sky-200'
            : 'border-slate-200 hover:border-sky-300 hover:bg-slate-50'; ?>">
          <div class="flex items-center justify-between">
            <span class="text-[11px] font-bold uppercase tracking-wide sm:text-xs <?php echo $aktif ? 'text-sky-700' : 'text-slate-500'; ?>"><?php echo $m['kd_mode']; ?></span>
            <?php if ($aktif): ?><i class="fa fa-check text-xs text-sky-600"></i><?php endif; ?>
          </div>
          <p class="mt-1 text-[13px] font-semibold leading-tight text-slate-800 sm:mt-1.5 sm:text-sm"><?php echo $m['nama_mode']; ?></p>
          <p class="mt-0.5 truncate text-[10px] leading-snug text-slate-400 sm:text-[11px] sm:text-slate-500"><?php echo $m['label_peserta']; ?> · <?php echo $m['label_staf']; ?></p>
        </button>
      <?php endforeach; ?>
    </div>
  ...
  <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
    ...
    <div class="mt-2.5 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
      <?php foreach (array('label_instansi','label_kepala','label_staf','label_peserta','label_nomor_induk','label_rombongan','label_mata_ajar','label_laporan','label_tahun','sistem_nilai') as $k): ?>
        <div class="rounded-lg bg-slate-50 px-2.5 py-1.5 sm:rounded-xl sm:px-3 sm:py-2">
          <p class="text-[9px] font-semibold uppercase tracking-wide text-slate-400 sm:text-[10px]"><?php echo $k; ?></p>
          <p class="truncate font-medium text-slate-700"><?php echo isset($mr[$k]) ? $mr[$k] : ''; ?></p>
        </div>
      <?php endforeach; ?>
    </div>
  </div>
