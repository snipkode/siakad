<div class="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
  <h3 class="mb-6 text-lg font-bold text-slate-800">Form Edit User</h3>

  <?php echo form_open_multipart('user/edit', 'role="form"'); ?>
  <?php echo form_hidden('id_user', $user['id_user']); ?>
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
      <div>
        ...
        <input type="text" value="<?php echo $user['nama_lengkap']; ?>" name="nama_lengkap" placeholder="Masukkan Nama Lengkap"
      ...
      <div>
        ...
        <input type="text" value="<?php echo $user['username']; ?>" name="username" placeholder="Masukkan Username"
               class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/30">
      </div>
      <div>
        <label class="mb-1.5 block text-sm font-medium text-slate-700">Password</label>
        <input type="password" value="<?php echo $user['password']; ?>" name="password" placeholder="Masukkan Password"
               class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/30">
      </div>
      <div>
      ...
      <div class="sm:col-span-2">
        ...
        <?php if (!empty($user['foto'])): ?>
          <img src="<?php echo base_url().'/uploads/user/'.$user['foto']; ?>" width="96" class="mt-3 rounded-xl border border-slate-200">
        <?php endif; ?>
      </div>
    </div>
