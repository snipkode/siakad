<div class="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
  <h3 class="mb-6 text-lg font-bold text-slate-800">Form Tambah Pembayaran</h3>

  <?php echo form_open('pembayaran/add', 'role="form"'); ?>
      ...
      <div>
        ...
        <select name="nim" required
          ...
          <?php foreach ($siswa as $s): ?>
            <option value="<?php echo $s->nim; ?>" <?php echo ($s->nim == '18SI1000') ? 'selected' : ''; ?>><?php echo $s->nim; ?> - <?php echo $s->nama; ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div>
        ...
        <select name="id_tahun_akademik" required
          ...
          <?php foreach ($tahun as $t): ?>
            <option value="<?php echo $t->id_tahun_akademik; ?>" <?php echo ($t->id_tahun_akademik == $aktif) ? 'selected' : ''; ?>><?php echo $t->tahun_akademik; ?> (<?php echo $t->semester; ?>)<?php echo ($t->is_aktif == 'Y') ? ' - Aktif' : ''; ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </div>
    ...
    <div class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2">
      <div>
        ...
        <input type="text" name="jenis_bayar" list="jenis-list" placeholder="cth: Pendaftaran PSB" required
      ...
      <div>
        ...
        <input type="number" name="jumlah" min="0" step="500" placeholder="cth: 250000" required
               class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
      </div>
    </div>
    ...
    <div class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2">
      <div>
        ...
        <input type="date" name="tanggal_bayar" value="<?php echo date('Y-m-d'); ?>" required
               class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
      </div>
      <div>
        <label class="mb-1.5 block text-sm font-medium text-slate-700">Keterangan</label>
        <input type="text" name="keterangan" placeholder="cth: Pelunasan / Angsuran 1"
               class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
      </div>
    </div>
