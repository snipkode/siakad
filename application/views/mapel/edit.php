<div class="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
  <h3 class="mb-6 text-lg font-bold text-slate-800">Form Edit <?php echo meta_mode_label('label_mata_ajar'); ?></h3>

  <?php echo form_open('mapel/edit', 'role="form"'); ?>

    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
      <div>
        <label class="mb-1.5 block text-sm font-medium text-slate-700">Kode <?php echo meta_mode_label('label_mata_ajar'); ?></label>
        <input type="text" value="<?php echo $mapel['kd_mapel']; ?>" readonly name="kd_mapel"
               class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-500">
      </div>
      <div>
        <label class="mb-1.5 block text-sm font-medium text-slate-700">Nama <?php echo meta_mode_label('label_mata_ajar'); ?></label>
        <input type="text" value="<?php echo $mapel['nama_mapel']; ?>" name="nama_mapel" placeholder="Masukkan Nama <?php echo meta_mode_label('label_mata_ajar'); ?>"
