<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
  <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-5">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h3 class="text-sm font-bold text-slate-800">Data Jurusan</h3>
        <p class="text-xs text-slate-500">
          Total
          <span id="total-jurusan" class="inline-flex items-center justify-center rounded-full bg-indigo-100 text-indigo-700 px-2 py-0.5 text-[11px] font-bold">0</span>
          terdaftar
        </p>
      </div>
    </div>
    <div class="flex flex-wrap gap-2 dt-actions">
      <?php
    echo anchor('jurusan/add', '<i class="fa fa-plus"></i> Tambah Data', array('class'=>'inline-flex items-center gap-1.5 rounded-xl bg-sky-600 px-3 py-2 text-xs font-semibold text-white shadow-sm hover:bg-sky-700'));
  ?>
    </div>
  </div>
...
{ "data": null, "width": "45px", "class": "text-center", "orderable": false },
    ...
    t.on('draw.dt', function () {
      $('#total-jurusan').text(t.page.info().recordsTotal);
